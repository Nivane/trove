...
Trove 是**自学习型对话数据代理**：自然语言提问，返回有真实 SQL 支撑的 Markdown 答案。它自动完成 schema 匹配、SQL 生成与自校验、执行与反思裁决；答错时诊断根因、回滚并修正。

每次问答与修正都沉淀为该数据源专属的知识库（注释、术语、参考 SQL、规则、经验教训）和跨会话记忆——**越用越准**。

**语义优先**：语义模型（`semantics.yml`）定义唯一可答边界——未覆盖的查询被拒绝并给出扩展模型的建议，绝不凭猜测作答。
