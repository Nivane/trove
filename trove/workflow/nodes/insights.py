...
def make_insights(
    ...
    async def insights(state: WorkflowState) -> dict[str, Any]:
        ...
        rows_note = ""
        if state.row_count > MAX_INSIGHT_ROWS:
            rows_note = (
                "Note: only the first "
                f"{MAX_INSIGHT_ROWS} of {state.row_count} rows are shown, in "
                ...
                f"注意：以下仅展示 {state.row_count} 行中的前 {MAX_INSIGHT_ROWS} 行，"
                "为查询返回顺序（可能未排序）——除非 SQL 已按其排序，不要仅凭预览"
                "推断全局最大/最小值或极值。"
            )
        model = config.model_for_node("insights", state.complexity)
        try:
