...
def register_kb_commands(registry: SlashRegistry, context: dict) -> None:
    ...
    def _report_assets(kb, datasource: str) -> str:
        ...
        for asset in report:
            ...
            lines.append(
                ...
                f"generated by {asset['generator'] or 'unknown'}"
            )
    ...
    async def cmd_kb(args: str) -> str:
        ...
        if sub == "migrate":
            # C1:只做体检(--check)。真正的迁移是 C2 的合并写路径;在那之前
            # 迁移 KB 资产的显式动作仍是 /kb init --overwrite。
            if "--check" not in args:
                return (
                    "Usage: /kb migrate --check\n"
                    "  --check  report each asset's content-format version and whether "
                    "it carries human edits (read-only; nothing is written)\n"
                    "Applying a migration arrives with the three-way merge; today "
                    "/kb init --overwrite regenerates from scratch."
                )
            await kb.ensure_synced(datasource or None)
            targets = (
            ...
            if not targets:
                return "No datasources with a KB. Run /kb init first."
        ...
        return (
            "Usage: /kb init [--docs <dir>] [--lang en|zh] [--overwrite] | list | reload | "
            "learn [--yes] | lessons [--yes] | migrate --check\n"
            ...
            "  migrate  --check reports each asset's format version and whether it "
            "carries human edits (read-only)"
        )
