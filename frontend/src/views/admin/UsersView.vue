<template>
  <div class="admin-view">
    <div class="view-header">
      ...
      <el-button type="primary" @click="openCreate">{{ t('createUser', ui.lang) }}</el-button>
    </div>

    <el-table :data="users" v-loading="loading" class="users-table">
      <el-table-column prop="id" label="ID" width="70" />
      <el-table-column prop="username" :label="t('username', ui.lang)" min-width="140" />
      <el-table-column prop="display_name" :label="t('displayName', ui.lang)" min-width="140" />
      <el-table-column prop="role" :label="t('role', ui.lang)" width="90" />
      <el-table-column :label="t('disabled', ui.lang)" width="90">
        <template #default="{ row }">
          <el-tag :type="row.disabled ? 'danger' : 'success'">{{ row.disabled ? '✓' : '–' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="t('datasources', ui.lang)" min-width="160">
        <template #default="{ row }">
          ...
          >
            <el-option v-for="d in knownDatasources" :key="d" :label="d" :value="d" />
          </el-select>
        </template>
      </el-table-column>
      <el-table-column :label="t('actions', ui.lang)" width="300" fixed="right">
        <template #default="{ row }">
          <el-button size="small" @click="resetPassword(row)">{{ t('resetPassword', ui.lang) }}</el-button>
          <el-button size="small" @click="toggleDisabled(row)">
            {{ row.disabled ? t('enable', ui.lang) : t('disable', ui.lang) }}
          </el-button>
          <el-button size="small" type="danger" @click="del(row)">{{ t('deleteUser', ui.lang) }}</el-button>
        </template>
      </el-table-column>
    </el-table>

    <el-dialog v-model="createOpen" :title="t('createUser', ui.lang)" width="420">
      <el-form label-width="90">
        ...
        <el-form-item :label="t('role', ui.lang)">
          <el-switch v-model="createForm.isAdmin" active-text="admin" inactive-text="user" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const createForm = reactive({ username: '', password: '', display_name: '', isAdmin: false })
...
async function load() {
  ...
  try {
    ...
    knownDatasources.value = (ds.datasources ?? []).map((d: { name: string }) => d.name)
    users.value = []
...
function openCreate() {
  Object.assign(createForm, { username: '', password: '', display_name: '', isAdmin: false })
...
async function resetPassword(row: UserRow) {
  try {
    const { value } = await ElMessageBox.prompt('New password', 'Reset password')
