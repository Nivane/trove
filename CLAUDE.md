...
Trove is an NL→SQL conversational data agent: natural-language question → LangGraph pipeline (intent routing, semantic matching, SQL generation with self-validation, execution, reflection adjudication, failure rollback & correction) → Markdown answer. **语义优先（Phase B，决策 1-4）**：语义模型（`semantics.yml`）是生成的主视角与可答边界——gen_sql/query_sketch 以语义视角为主；编译边界是**分级逃生梯**（非全有全无拒绝）：硬 MISS（结构性：fan-out/join 二义/未覆盖表/坏派生定义）才拒绝 + 反问扩展模型（LLM 草拟 draft → 管理端确认 → 重答），软 MISS（词表/值/口径未声明：未声明 metric/字段/枚举值/时间分桶）产出 `PartialCompile` 骨架——可解析部分权威化（join/过滤/分组，执行前骨架保真校验兜底）、未解析组件交 gen_sql 按 plan 文本补齐，回答照常交付而非硬停；无语义模型的数据源整体拒绝并提示先 `/kb init`。catalog 枚举工具（`search_values` / `lookup_schema` / `explain_plan`）在 standard/complex 档懒注册（analyst/admin 角色可用），agent 运行时可触达物理 schema 用于值确认 / 结构懒加载 / 执行计划（仅作生成辅助，不替代语义边界判定）。产品定位从「开箱即用」改为「**接入即建模、建模即保障**」（conf/agent.yml `semantic_first: true` 默认开）。Questions and corrections accrue into a per-datasource knowledge base (KB); accuracy grows with use. Ships with a built-in BIRD financial demo datasource (`trove/demo.py`, schema identical to the official BIRD export).
...
- **默认业务栈 = PostgreSQL**：`postgres` 服务用 `paradedb/paradedb:latest` 镜像（业务表 + pgvector 向量 + pg_bm25 全文检索同实例，`fts_tokenizer` 面向中文 KB 检索），`db-init` 一次性服务把 BIRD 金融 demo 数据灌入（幂等，`scripts/init_postgres_demo.py`），后端 `--datasource postgres://trove:trove@postgres:5432/trove`；向量后端默认 `pgvector`（`vector_dsn` 留空 = 同实例推导）。换回内置 SQLite demo 可改 `--datasource demo`，生产数据源改由管理端注册（`/admin/datasources`，持久化到 `.trove/datasources.yml`，重启自动恢复）
...
- **`graphs.py`** — three workflows: `reflection` (default, with self-correction loop) / `fixed` (direct pass) / `empty` (debug pass-through). Main query pipeline: `route_intent → parse_date → schema_linking → (refuse/clarify 语义门禁) → fast_match 确定性快径 → query_sketch(条件节点) → gen_sql[subgraph] → semantics → hitl(可选人工确认) → execute_sql → select → validate → reflect → (失败 → analyze_error 回滚) → insights → chart → conclusion → output`. Intent routing splits query and metadata paths (the latter runs `answer_metadata → metadata_check` self-validation loop); `query_sketch` is a conditional node (skipped when the graph is built without it).
- **Node pattern**: nodes are `make_<name>(services...)` factories returning `async def node(state) -> dict` (partial state update; passes through untouched when `state.error` is set); the terminal answer nodes (`output` / `answer_reject` / `answer_chitchat` / `answer_correction`) are plain module-level async functions. Services are bound into closures at graph build time (`GraphServices`).
...
- Tools come from a registry factory: `build_sql_registry(connectors, question, lang, dialect, *, finish=True, matched_tables=None, datasource="") -> ToolRegistry` — check hits accumulate on `registry.check_hits` (no positional return; legacy `make_sql_tools` kept for tests returns a tuple) — `validate_sql` (SQLGlot syntax, always available), `probe_query` (read-only execution observation, 10 rows/5s), `check_result` (read-only execution then `rules.verify` rule chain), plus the explicit `finish(answer)` tool (SQL delivered as its payload), and the catalog tools `search_values` / `lookup_schema` / `explain_plan` — registered only on standard/complex tiers, lazy + role-gated (analyst/admin); the agent may reach the physical schema for value confirmation / lazy schema load / execution plan as generation aids. probe/check share the `_probe_result` execution channel; check hits merge into `validation_hits` via the node update for eval attribution.
...
- **YAML is the single source of truth** (`.trove/kb/<datasource>/`: `schema_notes.yml`, `semantics.yml`, `examples.yml`, `rules.yml` (human-authored only — no writer), `lessons.yml`); the SQLite mirror is for runtime retrieval only. Datasource name = database name for network URLs (SQLite file datasources are named `sqlite`; the demo is `demo`).
...
- **`episode.py`** — **episodic memory** (P1): cross-session `question → SQL → verdict → correction_history → matched_tables` recall in `~/.trove/memory/episodes.sqlite`, deduped by `(datasource, user_id, question, sql)`. Read path = hybrid OR gate by default — lexical `relevance_score` ≥0.5 **or** cosine ≥0.55 when the datasource has an embedder; score = 0.6·cos + 0.4·lexical + 0.2·recency decay, zero LLM (rows carry an `embedding` BLOB). Injected into gen_sql as the `episodes` context block (success SQL = few-shot anchor; failure + corrections = counter-example).
...
- **`promotion.py`** — **auto-promotion** (P3, opt-in `memory.promotion: true`): confidence accumulator over evidence — currently only `repeated_correction` is wired to a call site (`_EVIDENCE_DELTA` also defines upvote / lesson-reuse deltas); crossing `promotion_threshold` auto-confirms a pending lesson via `KbService.update_lesson_confidence` (still human-auditable/revertible in `lessons.yml`).
...
- **`lifecycle`** (in `service.run_lifecycle`) — retention purge for episodes/preferences/user-facts (`UserFactsService.purge_expired`, previously dead code — now wired into the periodic sweep) / retrieval query log (only when `memory.retention_days` is set — default empty = 不清理); plus `schema_drift.py` comparing live catalog vs KB `schema_notes.yml` columns (zero LLM).
...
- `postgres.py` — `PostgresBackend` (production): translates SQLite idioms — `?`→`%s`, `INTEGER PRIMARY KEY AUTOINCREMENT`→`IDENTITY`, `INSERT OR REPLACE`→`INSERT` (ON CONFLICT clauses are authored directly in store SQL so they work on both backends), `lastrowid` via explicit `need_lastrowid=True` → `INSERT ... RETURNING id`. Rows support both name and position access.
...
- Internal stores call `resolve_backend(...)` with their storage path (SessionStore derives it from `home_dir`; TaskStore takes a backend object directly, with `resolve_backend` in its `from_db_path` classmethod) so tests pass tmp paths unchanged.
...
- **`trove/storage/migrations.py`** — versioned migrations for every internal store. `trove_schema(store, version, updated_at)` records where each store is (one physical DB holds several stores); `apply_migrations(target, store, migrations, dialect=...)` takes anything with `execute`/`commit` (a `StorageBackend`, a raw `aiosqlite`/psycopg connection — the KB mirror and retrieval stores are deliberately off `StorageBackend` but still need "which version am I"). Two invariants: **every op is individually idempotent** (that's what makes legacy adoption = replay-from-0 work, so fresh / legacy / retry-after-failure all take one code path) and **the version reflects history, never config** (optional config-driven columns go through `ensure_column` with no version row — otherwise flipping a config would make the DB look "newer" or silently skip a needed migration). Each step is one transaction with an explicit `BEGIN` (SQLite's implicit transaction doesn't cover DDL); a failed step rolls back and does **not** advance the version. A store newer than the code understands raises `StorageSchemaTooNew` — old code opening a new DB is the most silent kind of divergence (`SELECT` missing a few columns never errors, it just loses data). Derived artifacts (the KB mirror) **rebuild** rather than migrate, with the adoption probe happening exactly once. Absorbed sites: episodes table + `embedding` column, retrieval `documents` + `sparse` (both dialects — the sparse *channel* was later retired but the column and its migration step stay: dropping the step would make legacy DBs at v2 look newer than the code → `StorageSchemaTooNew`, and a destructive column drop buys nothing).

### LLM and tests

- `trove/llm/agent_loop.py` — shared agent-loop harness (`run_agent_loop`): parallel tool dispatch, per-tool timeout/retry, round/time/token budgets, loop-steering feedback, and a `ToolRegistry` (defs + handlers + observer middleware hooks: tracing happens here). The `finish(answer)` protocol lets the model deliver a validated final payload instead of relying on "no tool calls = done"; guard/empty results call back into the caller's degradation chain (gen_sql → classic subgraph). The query_sketch node is not agentic — it calls plain `llm.chat` with JSON output. `chat_full` returns best-effort `usage` for token budgets.
...
- **Debug query_sketch first**: wrong output columns from gen_sql usually root in the query_sketch node's `answer_columns` dictating output columns (the "Query plan (follow it...)" line in the user message outranks system rules; gen just obeys). Check the query_sketch node's output in `~/.trove/runs/<run_id>.log` (look up run_id from `.trove/eval/results.jsonl` by question).
